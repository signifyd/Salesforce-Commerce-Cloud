...
describe('signifyd', function () {
    ...
    describe('Callback', function () {
        ...
        before(function () {
            ...
            return request(myRequest).then(function (response) {
                ...
                .then(function (csrfResponse) {
                    csrfJsonResponse = JSON.parse(csrfResponse.body);
                    // Submit billing request with token aquired
        ...
        it('should update the order properties', function () {
            ...
            return request(myRequest)
                .then(function (response) {
                    csrfResponse = JSON.parse(response.body);
                    var URL = config.baseUrl + '/Signifyd-Callback?' +
                        csrfResponse.csrf.tokenName + '=' +
    ...
    describe('Test', function () {
        it('should return the caseId', function () {
            ...
            return request(myRequest).then(function (response) {
                var csfResponse = JSON.parse(response.body);
                var url = config.baseUrl + '/Test-CaseID?orderNumber=' + orderNumber + '&' +
                    csfResponse.csrf.tokenName + '=' +
                    csfResponse.csrf.token;
                myRequest = {
                    url: url,
                    method: 'GET',
                    rejectUnauthorized: false,
